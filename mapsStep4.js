...
alert('🚀 LibMaps Step 4.6 Working!');
...
var debugMode = false; // Set to false in production - SUCCESS! Buttons working
...
var springyILS = {
    ...
    isDuplicateItem: function(newItem, existingItems) {
        ...
        for (var i = 0; i < existingItems.length; i++) {
            ...
            if (existing.call === newItem.call) {
                debugLog("STEP 4.2: Duplicate call number detected - Call: " + newItem.call + " (keeping first instance)");
                return true;
            }
        }
        return false;
    ...
    scrapeMobileEnterpriseStructure: function(items) {
        ...
        debugLog("Found " + allCallElements.length + " Enterprise call number elements");
        ...
        for (var j = 0; j < allCallElements.length; j++) {
            ...
            if (text === "Shelf Number" || text === "Call Number" || 
                ...
                debugLog("STEP 4.4: Skipping label element: " + text);
                continue;
            }
            ...
            if (text && text.length > 0) {
                ...
                debugLog("STEP 4.4: Keeping call number element: " + text);
            }
        }
        
        debugLog("STEP 4.4: After filtering, " + callElements.length + " actual call number elements remain");
        ...
        for (var i = 0; i < callElements.length; i++) {
            ...
            var titleText = springyILS.getTitle() ? springyMap.extractText(springyILS.getTitle()) : document.title;
                ...
                if (springyILS.isGloballyProcessed(callText, location, collection)) {
                    debugLog("✗ Enterprise mobile item " + (i + 1) + " call number already processed: " + callText);
                    continue;
            ...
            if (springyILS.isDuplicateItem(potentialItem, items)) {
                debugLog("✗ Enterprise mobile item " + (i + 1) + " is duplicate within current batch, skipping");
                continue;
            }
        ...
        debugLog("STEP 3: Enterprise mobile structure found " + items.length + " total unique items");
        return items;
    },
    
    scrapeDetailRows: function(items) {
        debugLog("STEP 4: Starting unified scraping with global duplicate prevention");
        
        // STEP 4 ENHANCEMENT: Try Enterprise mobile structure FIRST on mobile
        if (isMobileDevice) {
            debugLog("STEP 4: Mobile detected - trying Enterprise mobile structure first");
            items = springyILS.scrapeMobileEnterpriseStructure(items);
            debugLog("STEP 4: Enterprise mobile structure found " + items.length + " items");
        }
        ...
        if (!rows || rows.length === 0) {
            debugLog("STEP 4: No traditional rows found - using items from mobile structure: " + items.length);
            return items;
        }
        
        debugLog("STEP 4: Processing " + rows.length + " traditional rows");
        ...
        console.log(`ENTERPRISE: Scraped ${items.length} total unique items`);
        return items;
    },
    ...
    attachButton: function(item, buttonDiv) {
        ...
        // STEP 4.3 ENHANCEMENT: Detailed button attachment debugging
        var targetElement = item.buttonElement || item.element;
        debugLog("STEP 4.3: Attaching button for call: " + item.call);
        debugLog("STEP 4.3: Target element class: " + (targetElement ? targetElement.className : 'no target'));
        debugLog("STEP 4.3: Target element tag: " + (targetElement ? targetElement.tagName : 'no target'));
        
        // Check if button already exists to prevent duplicates
        var existingButton = targetElement ? targetElement.querySelector('.springy-button-div') : null;
        if (existingButton) {
            debugLog("STEP 4.3: Button already exists for " + item.call + ", skipping attachment");
            return;
        }
        
        if (targetElement) {
            targetElement.appendChild(buttonDiv);
            debugLog("STEP 4.3: Button successfully attached for " + item.call);
        } else {
            debugLog("STEP 4.3: ERROR - No target element for " + item.call);
        }
    },
...
var springyMap = {
    ...
    setupButtons: function(items) {
        ...
        // STEP 4 ENHANCEMENT: Report unique items count
        debugLog("STEP 4.1: Processing " + items.length + " unique items for button creation");
        
        // STEP 4.3: Debug each item in detail
        debugLog("=== STEP 4.3: DETAILED ITEM ANALYSIS ===");
        for (let i = 0; i < items.length; i++) {
            ...
            debugLog("  Title: '" + item.title + "'");
            debugLog("  ButtonElement class: " + (item.buttonElement ? item.buttonElement.className : 'no element'));
            debugLog("  Element HTML: " + (item.element ? item.element.outerHTML.substring(0, 200) + "..." : 'no element'));
            debugLog("  ---");
        }
        debugLog("=== END ITEM ANALYSIS ===");
        ...
        for (let i = 0; i < items.length; i++) {
            ...
            if (item.call.length !== 0 && 
                ...
                debugLog("STEP 4.1: Creating button for call: " + item.call);
        ...
        debugLog("=== STEP 4.3: BUTTON CREATION SUMMARY ===");
        debugLog("Total items processed: " + items.length);
        debugLog("Expected buttons created: " + items.length);
        
        // Count actual buttons on page
        setTimeout(function() {
            var actualButtons = document.querySelectorAll('.springy-button-div').length;
            debugLog("ACTUAL buttons found on page: " + actualButtons);
            if (actualButtons > items.length) {
                debugLog("WARNING: More buttons than items - possible duplicate attachment!");
            }
        }, 100);
        
        debugLog("STEP 4.3: Button setup completed");
    },
