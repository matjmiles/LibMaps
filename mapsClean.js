// sirsiDynix Enterprise - Springs Lib Maps Integration - MOBILE ENHANCED VERSION
// Based on working mapsOld.js with mobile compatibility additions
...
function debugLog(message, data) {
    if (debugMode) {
        ...
        // Enhanced mobile debug overlay for better visibility on mobile
        if (isMobileDevice) {
            ...
            if (!document.getElementById('libmaps-debug')) {
                ...
                debugDiv.style.cssText = 'position:fixed;top:10px;right:10px;background:rgba(0,0,0,0.9);color:white;padding:10px;font-size:12px;z-index:99999;max-width:300px;max-height:200px;overflow-y:auto;border-radius:5px;border:2px solid #00ff00;';
...
var springyILS = {
    getTitle: function(element) {
        // Original working title selectors
        var titleSelectors = [
    ...
    scrapeDetailRows: function(items) {
        ...
        // MOBILE ENHANCEMENT: Updated selectors to work with Enterprise mobile structure
        var selectors = [
        ...
        // MOBILE ENHANCEMENT: If no traditional rows found, try Enterprise mobile structure
        if (!rows || rows.length === 0) {
            debugLog("No traditional detail rows found, trying Enterprise mobile approach...");
            
            // Find call number elements directly (discovered during mobile debugging)
            var callElements = document.querySelectorAll('.detailItemsTable_CALLNUMBER, [class*="detailItemsTable_CALLNUMBER"], [class*="CALLNUMBER"]');
            debugLog("Found " + callElements.length + " call number elements");
            
            if (callElements.length > 0) {
                // Create virtual items from call number elements
                for (var ce = 0; ce < callElements.length; ce++) {
                    var callEl = callElements[ce];
                    ...
                    callEl.classList.add('libmaps-processed');
                    
                    // Look for library info near this call element
                    ...
                    // Use default library if none found
                    ...
                    debugLog("Creating mobile item: Call='" + callText + "', Library='" + libraryText + "'");
                    ...
                    items.push({
                        ...
                        call: callText,
                        title: "Book Details",
                        collection: "" // Empty for mobile version
                    });
                }
                
                debugLog("Created " + items.length + " items from mobile structure");
                return items;
            }
        ...
        // Original desktop processing for traditional rows
...
var springyMap = {
    ...
    createModal: function(item, params) {
        ...
        console.log("ENTERPRISE: Creating modal with URL:", url);
    ...
    watch: function() {
        console.log("ENTERPRISE: Starting DOM watcher...");
        
        let attempts = 0;
        // MOBILE ENHANCEMENT: More attempts and longer interval for mobile
        const maxAttempts = isMobileDevice ? 60 : 30;
        const interval = isMobileDevice ? 750 : 500;
        
        debugLog("Watch configuration - Max attempts: " + maxAttempts + ", interval: " + interval + "ms");
        
        const watcher = setInterval(function() {
            attempts++;
            console.log(`ENTERPRISE: Watch attempt ${attempts}/${maxAttempts}`);
            ...
            if (targetElement) {
                console.log("ENTERPRISE: Target elements found, initializing scrape...");
                clearInterval(watcher);
                
                // MOBILE ENHANCEMENT: Longer delay for mobile
                setTimeout(function() {
                    springyMap.scrape();
                }, isMobileDevice ? 1500 : 750);
                
            } else if (attempts >= maxAttempts) {
                console.warn("ENTERPRISE: Watch timeout - target elements not found");
                clearInterval(watcher);
                
                setTimeout(function() {
                    console.log("ENTERPRISE: Attempting generic scrape as fallback...");
