...
// Step 1: Desktop Working Version + Enhanced Mobile Debug System ONLY

alert('🚀 LibMaps Step 1 Starting!');
...
// STEP 1 ENHANCEMENT: Emergency debug system for mobile (from mapsDebugSafe.js)
...
// EXACT debugLog function from working mapsOld.js 
function debugLog(message, data) {
    if (debugMode) {
        ...
        if (isMobileDevice) {
            // STEP 1 ENHANCEMENT: Only add mobile debug when document.body exists
            if (document.body) {
...
// EXACT calls from mapsOld.js
debugLog("Mobile device detected: " + isMobileDevice);
debugLog("User agent: " + navigator.userAgent);

// ===== FROM HERE DOWN: EXACT COPY OF mapsOld.js (desktop working version) =====
...
var springyMap = {
    ...
    // STEP 1: Keep EXACT desktop timing from mapsOld.js (no mobile changes yet)
    watch: function() {
        console.log("ENTERPRISE: Starting DOM watcher...");
        
        let attempts = 0;
        const maxAttempts = 30; // Desktop timing only
        const interval = 500;   // Desktop timing only
        ...
        const watcher = setInterval(function() {
            ...
            if (targetElement) {
                ...
                // Add slight delay to ensure page is fully rendered
                setTimeout(function() {
                    springyMap.scrape();
                }, 750); // Desktop timing only
